...
struct ChatAttachment: Identifiable {
    ...
    /// Local file reference used by chat UI previews so Send does not need to
    /// synchronously encode/write image display data.
    var displayImageReference: String? = nil
...
struct ChatInputField: View {
    ...
    private var composerShell: some View {
        ...
        .shadow(
            color: theme.isDark
                ...
                : Color.black.opacity(isFocused ? 0.12 : 0.08),
            radius: theme.isDark ? (isFocused ? 7 : 5) : (isFocused ? 22 : 16),
            x: 0,
            y: theme.isDark ? 2 : 8
        )
    }
    ...
    private var composerBorderColor: Color {
        isFocused
            ? theme.brandPrimary.opacity(theme.isDark ? 0.42 : 0.28)
            : (theme.isDark ? Color.white.opacity(0.20) : Color.black.opacity(0.045))
    }
