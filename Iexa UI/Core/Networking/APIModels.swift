...
struct ChatCompletionRequest: Sendable {
    ...
    /// gateways reject hosted tool types on `/chat/completions`.
    var responsesTools: [[String: Any]]?
    /// Responses tool choice. Can be a string (`"auto"`/`"none"`) or a hosted
    /// tool object such as `["type": "image_generation"]`.
    var responsesToolChoice: Any?
